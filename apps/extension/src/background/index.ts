  ...
  try {
    switch (msg.type) {
      ...
      case "VERIFY_AND_UNLOCK_VAULT": {
        ...
        if (!metaRes.ok) return { needsSetup: true }
      ...
      case "GENERATE_PASSWORD": {
        return { password: generateSecurePassword(16) }
      }
      ...
      case "GENERATE_AND_SAVE_PASSWORD": {
        ...
        const { name, url, username, email, password } = msg.payload || {}
        ...
        if (!res.ok) return { error: "Failed to save password" }
        return { success: true }
      }
      ...
      case "AUTOFILL_CURRENT_SITE": {
        ...
        if (!res.ok) return { error: "Failed to fetch vault" }
